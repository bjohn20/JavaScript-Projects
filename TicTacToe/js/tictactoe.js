...
function placeXOrO(squareNumber) {
  ...
  // selectSquare array to see if it contains the square number
  // clicked on
  if (!selectedSquares.some((element) => element.includes(squareNumber))) {
    //This variable retrieves the html element id that was
    //clicked.
    let select = document.getElementById(squareNumber);
    //This condition checks who's turn it is and
    if (activePlayer === "X") {
      //If activePlayer is equal to'X', the x.png is placed in
      //HTML
      select.style.backgroundImage = 'url("./images/x2.jpg")';
      // Active player may only be 'X' or 'O' so, if not 'X' it must be 'O'
    } else {
      // If activePlayer is equal to 'O', the o.png is placed in
      // HTML
      select.style.backgroundImage = 'url("./images/Oring2.jpg")';
    }
  ...
  // This function results in a random square being selected by
  // the computer
  function computersTurn() {
    ...
    let pickASquare;
    // This conditon allows our while loop to keep trying if a
...
// win conditions
// drawLine()  function is called to draw a line on the screen if
...
function checkWinConditions() {
  ...
  // This condtion checks for a tie. If non of the above
  // conditons are met and 9 squares are selected the code
  // executes
  else if (selectedSquares.length >= 9) {
    // This function plays the tie game sound
    audio("./media/tie_2.wav");
  ...
  // This function checks if an array includes 3 strings. It is
...
// earlier for placement sound('./media/place.mp3')
function audio(audioURL) {
  // We create a new audio object and we pass the path as a
  // parameter
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
  ...
  // This line gives is access to methods and properties to use
  // on canvas
  ...
  animateLineDrawing();
  // This line waits 1 second. Then, clears canvas, resets
  // game, an allows clicking again
  setTimeout(function () {
    clear();
    resetGame();
  }, 1000);
